...
import {getCurrentCurrencyQuery,getCurrentCategoryQuery,currentCategory,getCurrentProductQuery,getCartItemsQuery,getOverlayQuery,currentProduct,cartItems,overlay} from '../../graphql/reactivities/state'
        ...
        <StyledCategory overlay={overlay()}>
         <StyledCategoryName>
            ...
            {category.products.slice((page-1)*6,page*6).map(item=>{
              return (
                <StyledProductsList key={item.id} onClick={(e)=>this.setProduct(item)}>
            ...
            <StyledProductsPagination>
              {this.state.currentPage>1 && <li style={{cursor:"pointer"}} onClick={(e)=>{this.setState({currentPage: this.state.currentPage-1})}}>{'<'}</li>}
             {
               Array.from(Array(this.state.pageNumber).keys()).map((page)=>{
                 return (
                   <StyledProductsPages active={this.state.currentPage===page+1} key={page} onClick={(e)=>{this.setState({currentPage: page+1})}}>{page+1}</StyledProductsPages>
                 )
               }
               )
            ...
            {this.state.currentPage<this.state.pageNumber && <li style={{cursor:"pointer"}} onClick={(e)=>{this.setState({currentPage: this.state.currentPage+1})}}>{'>'}</li>}
...
setProduct(obj){
  ...
  render(){
    return(
    <Layout>
      {this.displayCategoryDetails(this.state.currentPage)}
    </Layout>
    )
  }
}
...
const StyledCategory = styled.div`
  ...
  height:auto;
  margin:auto;
  background-color:${props =>props.overlay?"rgba(57, 55, 72, 0.22)": "#FFFFFF"}; 
`
...
width:20px;
height:20px;
background-color:${props=>props.active?"rgba(57, 55, 72, 0.22)":""};
text-align:center;
border-radius:50%;
`