      ...
      <StyledProductDetails>
          ...
          <StyledProductGalleryContainer>
          {
            product.gallery.map((im)=>{
              return(
                <StyledProductGallery key={im} onClick={(e)=>{this.changeImage(im)}}>
                 <StyledProductGalleryImage  width="79px" height="80px" srcSet={im} alt="gallery"/>
        ...
        <StyledProductDetailsContainer>
          <StyledProductDetailsImageContainer>
             <StyledProductDetailsImage width="610px" height="511px" srcSet={currentProductDetailsImage()||product.gallery[0]} alt="gallery"/>
          </StyledProductDetailsImageContainer>
          <StyledProductDetailsContent>
                    ...
                    <StyledProductDetailsContentAttributeValueContainer>
                      {
                        attribute.type==="swatch"?
                          ...
                          {attribute.items.map((item)=>{
                            return(
                              <StyledProductDetailsContentAttributeValue onClick={(e)=>this.setAttributes(e,attribute.name,item.id,item.value)} key={item.id} 
                        ...
                        :
                         <StyledProductDetailsContentAttributeValueList>
                          ...
                          {attribute.items.map((item)=>{
                            return(
                              <StyledProductDetailsContentAttributeValue onClick={(e)=>this.setAttributes(e,attribute.name,item.id,item.value)} 
                                ...
                                key={item.id}>
                                <StyleProductDetailsContentAttributeValueItem>{item.value}</StyleProductDetailsContentAttributeValueItem>
                              </StyledProductDetailsContentAttributeValue>
                            )
                          })}
                          </StyledProductDetailsContentAttributeValueList>
           ...
           <StyledProductDetailsPrice>
             <StyleProductDetailsContentPriceTitle >PRICE</StyleProductDetailsContentPriceTitle>
             <StyledProductDetailsContentPriceValue>{currency.symbol} {product.prices.find((price)=>price.currency.label===currency.label).amount}</StyledProductDetailsContentPriceValue>
           </StyledProductDetailsPrice>
           <Link to="/products">
             <StyledButton onClick={()=>this.addToCart(product,this.state.attributes)}>ADD TO CART</StyledButton>
           </Link>
           <StyledProductDetailsDescription dangerouslySetInnerHTML={{ __html: product.description }} />
